...
domain: 高中数学竞赛组合
difficulty: 中学
knowledge_point: Combinatorial Optimization，Algebraic Combinatorics, Discrete Geometry, Lattice Theory
batch: 01_第一批100题
teacher: xieshuhong
---

## 题目
Let n be a positive integer. We define the scale parameter λ as the unique integer satisfying the binomial identity:
$$ \sum_{k=0}^{\lambda} \binom{\lambda}{k} = 4^{n} $$

Consider the lattice of Gaussian integers Λ = Z[i]. We define the bounded configuration space 𝒟_{λ} ⊂ Λ as the set of lattice points whose real and imaginary parts are bounded by half the scale parameter:
$$ \mathcal{D}_{\lambda} = \{ z \in \Lambda \; ;\; 2|\operatorname{Re}(z)| \le \lambda \ \text{and}\ 2|\operatorname{Im}(z)| \le \lambda \} $$

We define a symmetric relation ~ on 𝒟_{λ} based on the Euclidean distance. Two distinct points z_1, z_2 ∈ 𝒟_{λ} are related (z_1 ~ z_2) if and only if their squared Euclidean distance satisfies the isotropic step condition:
$$ |z_1 - z_2|^2 = 2 $$

Let 𝔈 be the set of all unordered pairs {z_1, z_2} ⊂ 𝒟_{λ} such that z_1 ~ z_2. We seek the maximum cardinality of a subset 𝓜 ⊂ 𝔈 subject to the following two geometric constraints:

Vertex Disjointness: For any distinct pairs e_1, e_2 ∈ 𝓜, their intersection is empty (e_1 ∩ e_2 = ∅).

Barycentric Injectivity: Let 𝔟: 𝔈 → ℂ be the barycenter map defined by 𝔟({u, v}) = (u+v)/2. The restriction of 𝔟 to the set 𝓜 must be injective. That is, for any distinct pairs e_1, e_2 ∈ 𝓜:
$$ \mathfrak{b}(e_1) \neq \mathfrak{b}(e_2) $$

Determine the value of |\mathcal{M}|_{\max} in terms of n.

答案：|\mathcal{M}|_{\max} = 2n^2 + 2n
